...
export default function VoiceModulatorPanel({ onProsodyChange, selectedLanguage = 'en' }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-3xl p-5 glass-panel-luxury space-y-5 shadow-2xl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-border">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-sm font-extrabold text-text">Interactive Voice Modulation & Acoustic Biomarkers</h3>
            <p className="text-[11px] text-text-muted">Test vocal tremor modulation, pitch instability & telephonic POTS filter</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all border flex items-center space-x-1.5 ${
              isSpeakingPrompt
                ...
                : 'bg-background hover:bg-border border-border text-text'
            }`}
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all border flex items-center space-x-1.5 ${
              isPlayingAudio
                ? 'bg-risk-critical text-white border-risk-critical animate-pulse'
                : 'bg-primary hover:bg-primary-dark text-white border-primary shadow-md'
            }`}
          >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-border">
        ...
        <div className="flex items-center space-x-2">
          <span className="text-xs text-text-muted font-semibold">Estimated Acoustic Stress:</span>
