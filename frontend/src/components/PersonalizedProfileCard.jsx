    ...
    <div className="bg-surface border border-border rounded-3xl p-6 glass-panel space-y-5 shadow-2xl animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-border">
        <div className="flex items-center space-x-3">
          <div className="p-2.5 bg-primary/10 border border-primary/30 rounded-2xl text-primary-dark">
            <Heart className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-base font-bold text-primary-dark">Your Support Plan</h3>
      ...
      {isPlanOpen && (
        <div className="bg-background p-5 rounded-2xl border border-border space-y-4 animate-fade-in">
          ...
          <div className="flex items-center justify-between border-b border-border pb-3">
            ...
            <span className="text-[10px] text-primary-dark bg-primary/10 px-2.5 py-0.5 rounded border border-primary/30 font-mono">
          ...
          {recommendations.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
              {recommendations.map((rec, idx) => (
                <div key={idx} className="bg-surface p-4 rounded-2xl border border-border space-y-2.5 shadow-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-primary-dark block">{rec.vertical}</span>
                  ...
                  <div className="space-y-1 text-[11px] text-text font-sans">
                    <p className="text-text font-medium">
                      <strong>What support is available:</strong> {rec.action_item}
                    </p>
                    <p className="text-text-muted italic">
                      <strong>Why it may be relevant:</strong> {rec.why}
                    </p>
                    <p className="text-primary-dark text-[10px] pt-1 border-t border-border/60">
                      <strong>What could happen next:</strong> A designated specialist from {rec.agency} may reach out to coordinate this option safely.
                    </p>
                  </div>
                </div>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-text">
              ...
              <div className="bg-surface p-4 rounded-xl border border-border space-y-2">
                <span className="text-xs font-semibold text-primary-dark block">1. Someone to Talk To</span>
                <p className="text-text-muted leading-relaxed text-[11px]">
                  Based on what you shared, speaking with a trained, trauma-informed counsellor may be helpful in <strong>{district}</strong>.
                </p>
              </div>
              ...
              <div className="bg-surface p-4 rounded-xl border border-border space-y-2">
                ...
                <p className="text-text-muted leading-relaxed text-[11px]">
                  Based on the situation you shared, legal assistance via the District Legal Services Authority (DLSA) may be relevant.
                </p>
              </div>

              <div className="bg-surface p-4 rounded-xl border border-border space-y-2">
                <span className="text-xs font-semibold text-risk-low block">3. Safety Review & Protection</span>
                <p className="text-text-muted leading-relaxed text-[11px]">
                  Because potential threats were mentioned, a human professional may need to review your safety situation under SC/ST Protection guidelines.
                </p>
              </div>
          ...
          <div className="p-4 bg-surface rounded-xl border border-border text-xs text-text space-y-2 font-sans">
            <div className="flex items-center space-x-1.5 text-primary-dark font-semibold">
